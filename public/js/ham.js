...
$(document).ready(function() {
	...
	function editBurger() {
		...
		let id = $(this).data('id');
		$(this)
			.siblings('.save-burger')
			.removeClass('d-none');
		let editText = $(`.todo-item-${id}`);
		let content = editText.text();
		editText.html(
			`<input type="text" class="form-control burger-item-${id}" value="${content}" placeholder="${content}" required="">`
		);
	}
